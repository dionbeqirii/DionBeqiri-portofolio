...
const techUsed = [
  "SuiteScript 2.x","SuiteFlow","Advanced PDF/HTML","REST APIs",
  "Saved Searches","JavaScript","SQL","NetSuite ERP",
];
...
export default function Experience() {
  ...
  return (
    <section id="experience" className="relative section-padding bg-[#0d1526]/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-slate-500 max-w-xl">{t.experience.subtitle}</p>
        </motion.div>
            ...
            <div className="p-6 sm:p-8 rounded-2xl bg-[#111827] border border-slate-800/60 card-glow group hover:border-emerald-500/20 transition-all duration-500">
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-white mb-1">{t.experience.role}</h3>
                    <p className="text-emerald-400 text-sm font-medium">{t.experience.company}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 self-start flex-shrink-0">
                  <Calendar size={14} />
                  <span className="text-sm font-semibold">{t.experience.duration}</span>
                </div>
              </div>
              ...
              <div className="space-y-4 mb-8">
                {t.experience.highlights.map((item, i) => (
                  <motion.div
                    key={i}
